...
def build_start_router(media_library: MediaLibraryService) -> Router:
    router = Router(name="start")

    @router.message(CommandStart())
    async def start(message: Message) -> None:
        profile = await media_library.get_profile(message.from_user.id)
        await message.answer(text(profile.language, "start"), reply_markup=main_menu(profile.language))
